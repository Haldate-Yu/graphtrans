  ...
  for nhead in 4 8 16; do
    for d_model in 64 128 256; do
      for num_encoders in 4 8 12; do
        python ../main.py --configs $config --num_workers 8 --device $cuda --batch_size $batch_size --nhead $nhead --d_model $d_model --gnn_emb_dim $d_model --num_encoder_layers $num_encoders
      done
    done
  done
done
